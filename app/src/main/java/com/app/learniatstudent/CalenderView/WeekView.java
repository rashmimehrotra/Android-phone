...
public class WeekView extends View {
    ...
    private TextPaint mEventTimerPaint;
    private Paint mHeaderColumnBackgroundPaint;
    public int mFetchedPeriod = -1; // the middle period the calendar has fetched.
    ...
    private int mNowLineColor = Color.parseColor("#FFCC00");
    private int mNowLineThickness = 5;
    private boolean mUseNewColoring = true;
    private int mHourSeparatorColor = Color.rgb(230, 230, 230);
    private int mTodayBackgroundColor = Color.rgb(239, 247, 254);
    private int mHourSeparatorHeight = 2;
    private int mTodayHeaderTextColor = Color.rgb(39, 137, 228);
    private int mEventTextSize = 24;
    private int mEventTextColor = Color.BLACK;
    private int mEventPadding = 10;
    private int mHeaderColumnBackgroundColor = Color.parseColor("#244463");//Color.WHITE;
    ...
    private int mEventCornerRadius = 5;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (mCurrentFlingDirection == Direction.HORIZONTAL){
                mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * 24  + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight()), 0);
            }
            else if (mCurrentFlingDirection == Direction.VERTICAL){
                mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * 24  + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight()), 0);
            }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (mEmptyViewClickListener != null && e.getX() > mHeaderColumnWidth && e.getY() > ( mHeaderRowPadding * 2 + mHeaderMarginBottom)) {
        ...
        public void onLongPress(MotionEvent e) {
            ...
            // If the tap was on in an empty space, then trigger the callback.
            if (mEmptyViewLongPressListener != null && e.getX() > mHeaderColumnWidth && e.getY() > ( mHeaderRowPadding * 2 + mHeaderMarginBottom)) {
                Calendar selectedTime = getTimeFromPoint(e.getX(), e.getY());
                if (selectedTime != null) {
                    performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // Hold references.
        mContext = context;
        face1= Typeface.createFromAsset(mContext.getAssets(), SLConstants.kFontRobotoRegular);
    ...
    private void init() {
        ...
        mHeaderMarginBottom = 1;//mTimeTextHeight / 2;
        initTextTimeWidth();
        ...
        mHourSeparatorPaint.setColor(Color.parseColor("#506982"));
        ...
        mEventBackgroundPaint.setShadowLayer(3, 1, 5, Color.parseColor("#50000000")); // shadow
        ...
        mEventTextPaint.setTextSize(34);
        mEventTextPaint.setTypeface(face1);

        // Prepare event timer size and color.
        mEventTimerPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG | Paint.LINEAR_TEXT_FLAG);
        mEventTimerPaint.setStyle(Paint.Style.FILL);
        mEventTimerPaint.setColor(mEventTextColor);
        mEventTimerPaint.setTextSize(mEventTextSize);
...
//                invalidate();
    ...
    private void drawCurrentTimeLine(Canvas canvas){
        ...
        canvas.drawLine(48, startY + beforeNow, startPixel + mWidthPerDay, startY + beforeNow, mNowLinePaint);
        Paint circlePaint = new Paint();
        circlePaint.setStyle(Paint.Style.FILL);
        circlePaint.setColor(Color.parseColor("#FFCC00"));
        canvas.drawCircle(46, startY + beforeNow, 8, circlePaint);
    }

    private void drawTimeColumnAndAxes(Canvas canvas) {
        // Draw the background color for the header column.
        canvas.drawRect(0,  mHeaderRowPadding * 0, mHeaderColumnWidth, getHeight(), mHeaderColumnBackgroundPaint);

        for (int i = 0; i < 24; i++) {
            float top =  mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * i + mHeaderMarginBottom;

            // Draw the text if its y position is not outside of the visible area. The pivot point of the text is the point at the bottom-right corner.
            String time = getDateTimeInterpreter().interpretTime(i);
            if (time == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null time");
            if(time.equalsIgnoreCase("12 PM") || time.equalsIgnoreCase("0 PM")){
                time = "noon";
            }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight= Math.max(mMinHourHeight, (int) ((getHeight()  - mHeaderRowPadding * 2 - mHeaderMarginBottom) / 24));
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * 24  - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2)
            mCurrentOrigin.y = getHeight() - mHourHeight * 24  - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2;
        ...
        float[] thirtyMinuteLines = new float[lineCount * 4];
...
//                    canvas.drawRect(start,  mHeaderRowPadding * 2 + mTimeTextHeight/2 + mHeaderMarginBottom, startPixel + mWidthPerDay, getHeight(), sameDay ? mTodayBackgroundPaint : mDayBackgroundPaint);
                }
            }
            ...
            for (int hourNumber = 0; hourNumber < 24; hourNumber++) {
                float top =  mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * hourNumber + mTimeTextHeight/2 + mHeaderMarginBottom;
                if (top >  mHeaderRowPadding * 2 + mTimeTextHeight/2 + mHeaderMarginBottom - mHourSeparatorHeight && top < getHeight() && startPixel + mWidthPerDay - start > 0){
                    hourLines[i * 4] = start;
                    hourLines[i * 4 + 1] = top;
            ...
            int j = 0;
            for (int hourNumber = 0; hourNumber < 24; hourNumber++) {
                float top =  mHeaderRowPadding * 5.1f + mCurrentOrigin.y + mHourHeight * hourNumber + mTimeTextHeight/2 + mHeaderMarginBottom;
                if (top >  mHeaderRowPadding * 5.1f + mTimeTextHeight/2 + mHeaderMarginBottom - mHourSeparatorHeight && top < getHeight() && startPixel + mWidthPerDay - start > 0){
                    thirtyMinuteLines[j * 4] = start;
                    thirtyMinuteLines[j * 4 + 1] = top;
                    thirtyMinuteLines[j * 4 + 2] = startPixel + mWidthPerDay;
                    thirtyMinuteLines[j * 4 + 3] = top;
                    j++;
                }
            }
            Paint fgPaintSel = new Paint();
            fgPaintSel.setAntiAlias(true);
            fgPaintSel.setDither(true);
            fgPaintSel.setColor(getResources().getColor(R.color.dashedline));
            fgPaintSel.setStyle(Paint.Style.STROKE);
            fgPaintSel.setPathEffect(new DashPathEffect(new float[]{10, 10, 10, 10}, 5));
            // Draw the lines for hours.
            canvas.drawLines(hourLines, mHourSeparatorPaint);
            canvas.drawLines(thirtyMinuteLines, fgPaintSel);
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    // Calculate top.
                    float top = mHourHeight * 24 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y  + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical;

                    // Calculate bottom.
                    float bottom = mEventRects.get(i).bottom;
                    bottom = mHourHeight * 24 * bottom / 1440 + mCurrentOrigin.y  + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - mEventMarginVertical;

                    // Calculate left and right.
                    float left = (startFromPixel + mEventRects.get(i).left * mWidthPerDay)+16;
                    if (left < startFromPixel)
                        left += mOverlappingEventGap;
                    float right = (left + mEventRects.get(i).width * mWidthPerDay)-32;
                    if (right < startFromPixel + mWidthPerDay)
                        right -= mOverlappingEventGap;

                    // Draw the event and the event name on top of it.
                    RectF eventRectF = new RectF(left, top, right, bottom);
                    if (bottom >  mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 && left < right &&
                            eventRectF.right > mHeaderColumnWidth &&
                            eventRectF.left < getWidth() &&
                            eventRectF.bottom >  mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom &&
                        ...
                        drawEventTimerText(mEventRects.get(i).event, mEventRects.get(i).rectF, canvas, top, left);
//                        drawEventTimerText1(mEventRects.get(i).event, mEventRects.get(i).rectF, canvas, top, left);
                    }
                    else
    ...
    private void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        String startTimeString = event.getTodaySessionModel().getStartTime();
        String[] startToken = startTimeString.split(" ");
        String[] startHourToken = startToken[1].split(":");
        String endTimeString = event.getTodaySessionModel().getEndTime();
        String[] endToken = endTimeString.split(" ");
        String[] endHourToken = endToken[1].split(":");
        String minutesDiff= CommonUtils._instance.getTimeDifference(startTimeString,endTimeString);
        if(Math.abs(Integer.parseInt(minutesDiff)) <=25){
            mEventTextPaint.setTextSize(18);
        }
        else {
            mEventTextPaint.setTextSize(34);
        }
        ...
        // Get text dimensions
        StaticLayout textLayout = new StaticLayout(bob, mEventTextPaint, availableWidth, Layout.Alignment.ALIGN_CENTER, 1.0f, 0.0f, false);
        ...
        if (availableHeight >= lineHeight) {
            ...
            canvas.save();
            if(Math.abs(Integer.parseInt(minutesDiff)) <=25){
                canvas.translate(originalLeft + 35, originalTop + 5);
            }
            else {
                canvas.translate(originalLeft + 35, originalTop + mEventPadding);
            }
     ...
     * @param originalLeft The original left position of the rectangle. The rectangle may have some of its portion outside of the visible area.
     */
    private void drawEventTimerText(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        if (rect.right - rect.left - mEventPadding * 2 < 0) return;
        if (rect.bottom - rect.top - mEventPadding * 2 < 0) return;

        SpannableStringBuilder bob = new SpannableStringBuilder();
        if (event.getName() != null) {
            bob.append(event.getTimerStaticText());
        ...
        // Get text dimensions
        DynamicLayout textLayout = new DynamicLayout(bob, mEventTimerPaint, availableWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        ...
        if (availableHeight >= lineHeight) {
            ...
            do {
                // ellipsize text to fit into event rect
                textLayout = new DynamicLayout(TextUtils.ellipsize(bob, mEventTimerPaint, availableLineCount * availableWidth, TextUtils.TruncateAt.END), mEventTimerPaint, (int) (rect.right - originalLeft - mEventPadding * 2), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                // reduce line count
                availableLineCount--;
                // and repeat until text is short enough
            } while (textLayout.getHeight() > availableHeight);
            ...
            // Draw text
            canvas.save();
            canvas.translate(originalLeft + mEventPadding, originalTop + 30);
            textLayout.draw(canvas);
            canvas.restore();
        }
    }

    private void drawEventTimerText1(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
...
//                sortAndCacheEvents(nextPeriodEvents);
    ...
    private void cacheEvent(WeekViewEvent event) {
        ...
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            WeekViewEvent event1 = new WeekViewEvent(event.getEventContext(),event.getId(), event.getName(), event.getLocation(), event.getStartTime(), endTime,event.getTodaySessionModel());
            ...
            while (!isSameDay(otherDay, event.getEndTime())) {
                ...
                WeekViewEvent eventMore = new WeekViewEvent(event.getEventContext(),event.getId(), event.getName(),event.getTimerStaticText(), overDay, endOfOverDay,event.getTodaySessionModel());
            ...
            WeekViewEvent event2 = new WeekViewEvent(event.getEventContext(),event.getId(), event.getName(), event.getLocation(), startTime, event.getEndTime(),event.getTodaySessionModel());
    ...
    public void sortAndCacheEvents(List<WeekViewEvent> events) {
        if(events == null){
            return;
        }
        sortEvents(events);
...
//                goToNearestOrigin();
    ...
    private void goToNearestOrigin(){
        ...
        mCurrentScrollDirection = mCurrentFlingDirection = Direction.VERTICAL;
    }
    ...
    public void goToHour(double hour){
        ...
        if (verticalOffset > mHourHeight * 24 - getHeight() +  mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int)(mHourHeight * 24 - getHeight() +  mHeaderRowPadding * 2 + mHeaderMarginBottom);
