...
public class WeekView extends View {
    ...
    private int mNowLineColor = Color.rgb(255, 255, 0);
...
//            if(mUseNewColoring && sameDay){
    ...
    private void drawEventTimerText1(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        TextView textView = new TextView(event.getEventContext());
...
//        }
    ...
    private void cacheEvent(WeekViewEvent event) {
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            WeekViewEvent event1 = new WeekViewEvent(event.getEventContext(),event.getId(), event.getName(), event.getLocation(), event.getStartTime(), endTime);
            ...
            while (!isSameDay(otherDay, event.getEndTime())) {
                ...
                WeekViewEvent eventMore = new WeekViewEvent(event.getEventContext(),event.getId(), event.getName(),event.getTimerStaticText(), overDay, endOfOverDay);
            ...
            WeekViewEvent event2 = new WeekViewEvent(event.getEventContext(),event.getId(), event.getName(), event.getLocation(), startTime, event.getEndTime());
